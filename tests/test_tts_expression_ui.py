...
class TTSExpressionUIContractTests(unittest.TestCase):
    def test_voice_settings_expose_expression_controls(self):
        ...
        for control in ('set-tts-tone', 'set-tts-intensity', 'set-tts-volume', 'set-tts-pause-style', 'set-tts-expression-detail'):
            self.assertIn(f'id="{control}"', html)
        for tone in ('neutral', 'calm', 'friendly', 'cheerful', 'serious', 'sad', 'angry', 'dramatic', 'narrator', 'auto'):
            self.assertIn(f'value="{tone}"', html)

    def test_edge_client_avoids_planning_round_trip_for_normal_short_speech(self):
        js = (ROOT / 'static' / 'js' / 'app.js').read_text(encoding='utf-8')
        self.assertIn('function edgeNeedsSpeechPlan', js)
        self.assertIn('!edgeNeedsSpeechPlan(clean)', js)
        self.assertIn('expression_detail:', js)
