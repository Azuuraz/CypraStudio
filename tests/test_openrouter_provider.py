...
class OpenRouterProviderContractTests(unittest.TestCase):
    def test_catalog_contains_current_verified_free_models_only(self):
        ...
        self.assertNotIn('thinkingmachines/inkling-small:free', by_id)
    ...
    def test_paid_models_are_not_available_in_the_catalog(self):
        from engine import openrouter
        self.assertNotIn('moonshotai/kimi-k2.5', {x['id'] for x in openrouter.list_models()})
    ...
    def test_stream_forces_utf8_when_sse_omits_charset(self):
        from engine import openrouter
        ...
        class FakeResponse:
            ...
            def iter_lines(self, decode_unicode=True):
                lines = [
                    'data: {"model":"openrouter/free","choices":[{"delta":{"content":"It’s clean — café ✓"},"finish_reason":null}]}'.encode('utf-8'),
                    b'data: [DONE]',
                ]
                for raw in lines:
                    yield raw.decode(self.encoding) if decode_unicode else raw

        settings = {'openrouter_allow_online': True, 'think_mode': 'auto', 'chat_temperature': 0.2, 'ollama_num_predict': 32}
        with patch.object(openrouter, 'load_api_key', return_value='sk-or-v1-testvalue'), \
             patch.object(openrouter.requests, 'post', return_value=FakeResponse()):
            events = list(openrouter.stream_chat(
                settings, [{'role':'user','content':'hi'}],
                model_override=openrouter.encode_model('openrouter/free'),
            ))

        self.assertIn(('content', 'It’s clean — café ✓'), events)
        self.assertNotIn('â', ''.join(str(payload) for kind, payload in events if kind == 'content'))

    def test_free_endpoint_rate_limit_falls_back_to_free_router(self):
        from engine import openrouter
        ...
        class FakeResponse:
            def __init__(self, status_code, lines=()):
                ...
                self.text = '{"error":{"code":429}}'
                self.lines = lines
            def __enter__(self): return self
            def __exit__(self, *args): return False
            def close(self): pass
            def iter_lines(self, decode_unicode=True): yield from self.lines
        ...
        settings = {'openrouter_allow_online': True, 'think_mode': 'auto', 'chat_temperature': 0.2, 'ollama_num_predict': 2}
        with patch.object(openrouter, 'load_api_key', return_value='sk-or-v1-testvalue'), patch.object(openrouter.requests, 'post', side_effect=fake_post):
            events = list(openrouter.stream_chat(settings, [{'role':'user','content':'hi'}], model_override=openrouter.encode_model('qwen/qwen3.8-27b:free')))

        self.assertEqual(calls, ['qwen/qwen3.8-27b:free', 'openrouter/free'])
